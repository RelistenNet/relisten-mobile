...
export default class Artist extends Model implements CopyableFromApi<ArtistWithCounts>, UpdatableFromApi, Favoritable {
    static table = 'artists';
    static associations = {
        user_list_entries: {type: 'has_many' as 'has_many', foreignKey: Columns.userListEntries.artistId},
        shows: {type: 'has_many' as 'has_many', foreignKey: Columns.shows.artistId},
        years: {type: 'has_many' as 'has_many', foreignKey: Columns.years.artistId},
    };
    ...
    @lazy onLists = this.collections.get<UserList>(Tables.userLists).query(
        Q.on(Tables.userListEntries, Columns.userListEntries.artistId, this.id)
    )
