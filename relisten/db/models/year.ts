...
export default class Year extends Model implements CopyableFromApi<ApiYear>, UpdatableFromApi, Favoritable {
    static table = 'years';
    static associations = {
        shows: {type: 'has_many' as 'has_many', foreignKey: Columns.shows.yearId},
    };
    ...
    @lazy onLists = this.collections.get<UserList>(Tables.userLists).query(
        Q.on(Tables.userListEntries, Columns.userListEntries.yearId, this.id)
    )
