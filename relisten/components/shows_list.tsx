...
const ShowListItem: React.FC<{ show: Show, isFavorite: boolean, onPress?: (show: Show) => {} }> =
    ({show, isFavorite, onPress}) => {
        const styles = useShowListItemStyles();

        return (<ListItem style={styles.listItem} onPress={onPress}>
...
const useShowListItemStyles = () => StyleSheet.create({
    listItem: {
        paddingHorizontal: 8,
        width: '100%'
    }
...
const ShowList: React.FC<{ shows: Favorited<Show>[], onItemPress?: (show: Show) => {} }> = ({shows, onItemPress}) => {
...
const enhanceShows = withObservables(['shows'], ({shows}: { shows: Observable<Show[] | undefined> }) => ({
