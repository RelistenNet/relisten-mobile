...
const ArtistsList: React.FC<{ artists: Favorited<Artist>[] } & NavigationProps> = ({
  artists,
  navigation,
  route,
}) => {
  ...
  return (
    <SectionList
      ...
      renderItem={({ item: artist }) => {
        return (
          <EnhancedArtistListItem artist={artist.model} navigation={navigation} route={route} />
        );
      }}
    />
  );
...
export const HomeScreen: React.FC<PropsWithChildren<{} & NavigationProps>> = ({
  ...
  return (
    <View useSafeArea flex style={{ width: '100%' }}>
      <EnhancedArtistsList artists={artists} navigation={navigation} route={route} />
    </View>
  );
};
