//
//  BASSLifecycle-enhanced.swift
//  RelistenAudioPlayer
//
//  Created by Alec Gorge on 7/13/23.
//  Enhanced with better error handling for timeouts
//
...
extension RelistenGaplessAudioPlayer {
    ...
    func maybeSetupBASS() {
        ...
        // Increase timeout from 15 to 30 seconds to handle slower connections better
    ...
    func streamStalled(_ stream: HSTREAM) {
        ...
        if stream == activeStream?.stream {
            ...
            // Check network connectivity when stalled
            checkNetworkAndMaybeRetry(forStream: stream)
        }
    }
    
    // New function to check network and retry if possible
    func checkNetworkAndMaybeRetry(forStream stream: HSTREAM) {
        // Only proceed if this is the active stream
        guard let activeStream = activeStream, stream == activeStream.stream else {
            return
        }
        ...
        monitor.pathUpdateHandler = { [weak self] path in
            ...
            DispatchQueue.main.async {
                ...
                let networkInfo: [String: Any] = [
                    ...
                    "interfaceTypes": interfaceTypes.map { String(describing: $0) },
                    "isExpensive": path.isExpensive,
                    "streamURL": activeStream.streamable.url.absoluteString
                ]
                
                if let delegate = self.delegate {
                    // If there's a network issue, send this to the app
                    if !isConnected {
                        let error = NSError(domain: "net.relisten.ios.network", 
                                           code: 1001, 
                                           userInfo: [
                                               NSLocalizedDescriptionKey: "Network connectivity issue detected",
                                               "networkInfo": networkInfo
                                           ])
                        delegate.errorStartingStream(self, error: error, forStreamable: activeStream.streamable)
                    }
                }
                
                // If we're connected but stalled, try to resume or restart the stream
                if isConnected && self.currentState == .Stalled {
                    self.bassQueue.async {
                        // Only attempt to restart if we're still stalled
                        if stream == self.activeStream?.stream && self.currentState == .Stalled {
                            // First try to resume
                            if BASS_ChannelPlay(stream, 0) == 1 {
                                self.currentState = .Playing
                            } else {
                                // If resume fails, try to restart with the same streamable
                                let streamable = activeStream.streamable
                                let currentPosition = self.elapsed ?? 0
                                
                                // Tear down and restart
                                self.tearDownStream(activeStream)
                                self.activeStream = nil
                                self.playStreamableImmediately(streamable, startingAtMs: Int64(currentPosition * 1000))
                            }
                        }
                    }
                }
    ...
    func createStreamWithRetry(url: URL, retryCount: Int = 0, maxRetries: Int = 2) -> (HSTREAM, NSError?) {
        // Try to create the stream
        let stream = BASS_StreamCreateURL(url.absoluteString, 0, DWORD(BASS_STREAM_PRESCAN), nil, nil)
        
        // Check for errors
        let errorCode = BASS_ErrorGetCode()
        if stream == 0 {
            let error = ErrorForErrorCode(errorCode)
            
            // If it's a timeout error and we haven't exceeded max retries
            if errorCode == BASS_ERROR_TIMEOUT && retryCount < maxRetries {
                // Log retry attempt
                print("[bass][stream] Timeout creating stream. Retry \(retryCount + 1)/\(maxRetries) for URL: \(url)")
                
                // Wait briefly before retry (exponential backoff)
                usleep(useconds_t(500_000 * pow(2, Double(retryCount))))
                
                // Retry with incremented count
                return createStreamWithRetry(url: url, retryCount: retryCount + 1, maxRetries: maxRetries)
            }
...
func ErrorForErrorCode(_ erro: Int32) -> NSError {
    ...
    if erro == BASS_ERROR_INIT {
        ...
        errorDetails["category"] = "network"
        errorDetails["timeout"] = BASS_GetConfig(DWORD(BASS_CONFIG_NET_TIMEOUT))
    ...
    return NSError(domain: "net.relisten.ios.relisten-audio-player", 
                  code: Int(erro), 
                  userInfo: [
