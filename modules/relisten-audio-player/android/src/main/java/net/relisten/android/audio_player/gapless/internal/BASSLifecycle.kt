...
class BASSLifecycle internal constructor(private val player: RelistenGaplessAudioPlayer) : BASS.SYNCPROC {
    ...
    internal fun maybeSetupBASS() {
        rwl.readLock().withLock {
            if (player.isSetup) {
                return@maybeSetupBASS
            }
        }

        rwl.writeLock().withLock {
            if (player.isSetup) {
                return
            }
            setupBASS()
    ...
    private fun setupBASS() {
        ...
        player.bass_assert("BASS_Init", BASS.BASS_Init(-1, 44100, 0))

        player.mixerMainStream = BASSmix.BASS_Mixer_StreamCreate(44100, 2, BASSmix.BASS_MIXER_END)

        player.bass_assert("BASS_Mixer_StreamCreate", player.mixerMainStream != null && player.mixerMainStream != 0)

        player.bass_assert(
                "mixerMainStream BASS_ChannelSetSync",
    ...
    internal fun mixInNextStream(completedStream: Int) {
        ...
        if (nextStream != null) {
            ...
            player.bass_assert("BASS_ChannelSetPosition", BASS.BASS_ChannelSetPosition(mixerMainStream, 0, BASS.BASS_POS_BYTE))
