...
The anonymous catalog client continues to use `https://api.relisten.net`. Favorites may call `POST /api/v3/catalog/resolve` when a membership lacks cached display metadata. One request accepts `source_track_uuids` plus typed `entity_refs` for artist, show, source, source track, song, tour, and venue. After deduplication, their combined total is at most 500. The response returns one typed status per requested reference and UUID-bearing arrays whose relationships use UUIDs. No account token is sent. Playlist reads do not use this endpoint; the Accounts API snapshot already includes the normalized catalog rows needed to render that playlist.
