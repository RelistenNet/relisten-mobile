...
| auth-session-user-service-client | active | root Codex agent | port 5119 is listening, but simple user-library probe paths returned 404 during the latest check; live auth/sync smoke still needs a healthy endpoint path | `docs/workstreams/backlog/auth-session-user-service-client/plan.md` | `docs/workstreams/backlog/auth-session-user-service-client/ledger.md` | branch `codex/scoped-realm-user-data` | Development-only sign-in surface is complete; run live local auth/sync/history smoke once the local user-library endpoint path is confirmed healthy. | `continue` |
...
| mobile-share-token-exchange | active | root Codex agent | live endpoint smoke waits on a confirmed healthy local `RelistenUserApi` share-token endpoint; Follow/Clone/editor UX deferred to playlist UX workstream | `docs/workstreams/backlog/mobile-share-token-exchange/plan.md` | `docs/workstreams/backlog/mobile-share-token-exchange/ledger.md` | branch `codex/scoped-realm-user-data` | Run live Universal Link/exchange smoke when server endpoint is healthy; reuse grant header helper in tokenless playlist reads once the read UI/client exists. | `continue` |
| playlist-sync-outbox | active | root Codex agent | post-local-write flush waits on playlist mutation adapters/UX; live smoke waits on local `RelistenUserApi` | `docs/workstreams/backlog/playlist-sync-outbox/plan.md` | `docs/workstreams/backlog/playlist-sync-outbox/ledger.md` | branch `codex/scoped-realm-user-data` | Pending operations now wake the lifecycle runner and 409 conflicts stay retryable; reuse the runner once playlist mutation adapters exist. | `continue` |
| favorites-sync-migration | active | root Codex agent | source/show/library-index scoped favorite reads remain deferred beyond the reusable heart-button path | `docs/workstreams/backlog/favorites-sync-migration/plan.md` | `docs/workstreams/backlog/favorites-sync-migration/ledger.md` | branch `codex/scoped-realm-user-data` | Extend scoped favorite reads into source/show/library selection paths, or explicitly defer those UI source-of-truth changes. | `continue` |
...
Queue V2, scoped Realm rows, token/session handling, active authenticated scope switching, Development-only sign-in UI, playlist pull-sync application, scoped favorite migration, signed-in reusable favorite-button mutation, playlist operation replay, authenticated lifecycle sync, signed-in playback history journaling/batch upload, Cast custom data/status reconciliation, CarPlay queue row identity, mobile share-token exchange/storage, and branch-wide secret/error hygiene now have enough pure behavior in place for local user-data workflows. Playlist UX has a concrete implementation map, but polished UI implementation needs product answers before mutation/edit/share/collaboration surfaces are built. The largest remaining scoped-data correctness gap is that source/show/library-index favorite consumers still read catalog `isFavorite` flags instead of scoped `UserFavorite` rows for signed-in users.

## Next Iteration

Claim `MOB-FAV-003` for source/show/library selection favorite consumers unless live local API health becomes available first. Both local ports were listening during the latest review-cleanup smoke, but catalog `/api/v3/artists` returned a server 500 (`prepared statement did not exist`) and the simple user-library probe paths returned 404, so Development auth/sync/history/share-link smoke remains deferred until the relevant local endpoints are healthy.
