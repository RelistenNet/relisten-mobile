...
Queue V2, scoped Realm rows, token/session handling, active authenticated scope switching, playlist pull-sync application, scoped favorite migration, playlist operation replay, and authenticated lifecycle sync now have enough pure behavior in place for local user-data workflows. The next risk is moving playback history into scoped batched upload without regressing anonymous playback reporting.

## Next Iteration

Commit `MOB-SYNC-003`, then promote `history-batch-upload-migration` unless playlist write adapters become the next priority. Run the live Development auth/sync smoke once `RelistenUserApi` is listening on `http://localhost:5119`.
