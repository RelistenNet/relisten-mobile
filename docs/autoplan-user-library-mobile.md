...
| playlist-sync-outbox | active | root Codex agent | app lifecycle/reconnect runner not wired yet | `docs/workstreams/backlog/playlist-sync-outbox/plan.md` | `docs/workstreams/backlog/playlist-sync-outbox/ledger.md` | branch `codex/scoped-realm-user-data` | Wire pull sync plus operation replay into authenticated app lifecycle after the runner boundary is chosen. | `continue` |
...
Queue V2, scoped Realm rows, token/session handling, active authenticated scope switching, playlist pull-sync application, scoped favorite migration, and playlist operation replay now have enough pure behavior in place for local user-data workflows. The next risk is wiring these services into app lifecycle/auth boundaries without starting sync while signed out or while another replay is in flight.

## Next Iteration

Commit `MOB-SYNC-002`, then choose between the authenticated sync runner/lifecycle slice and `history-batch-upload-migration`. Run the live Development auth smoke once `RelistenUserApi` is listening on `http://localhost:5119`.
