...
Commit `MOB-CAST-002`, then choose between share-token exchange, live local API smoke, or promoting playlist UX clarification. Run the live Development auth/sync/history smoke once `RelistenUserApi` is listening on `http://localhost:5119`.
