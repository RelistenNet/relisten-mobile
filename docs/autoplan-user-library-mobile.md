...
| history-batch-upload-migration | active | root Codex agent | live upload smoke waits on local `RelistenUserApi` | `docs/workstreams/backlog/history-batch-upload-migration/plan.md` | `docs/workstreams/backlog/history-batch-upload-migration/ledger.md` | branch `codex/scoped-realm-user-data` | Wire playback reporter journal writes and authenticated batch upload runner. | `continue` |
...
Queue V2, scoped Realm rows, token/session handling, active authenticated scope switching, playlist pull-sync application, scoped favorite migration, playlist operation replay, authenticated lifecycle sync, and scoped history batch DTOs now have enough pure behavior in place for local user-data workflows. The next risk is wiring the live playback reporter so signed-in plays are journaled and batch-uploaded without regressing anonymous playback reporting.

## Next Iteration

Commit `MOB-HIST-001`, then continue `history-batch-upload-migration` with reporter journal writes and authenticated batch upload flushing. Run the live Development auth/sync smoke once `RelistenUserApi` is listening on `http://localhost:5119`.
