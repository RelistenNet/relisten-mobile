...
Favorite resolver status is not a playback or download gate. Mobile prefers a downloaded file, otherwise it tries the cached stream URL and uses the ordinary player or download error path if that rare request fails. A later catalog refresh or missing-metadata hydration can restore usable metadata without changing favorite membership.
