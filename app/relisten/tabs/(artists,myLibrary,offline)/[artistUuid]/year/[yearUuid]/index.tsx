...
import { useArtistYearShows, useOfflineYearMetadata } from '@/relisten/realm/models/year_repo';
...
const YearHeader = ({ year }: { artist: Artist | null; year: Year | null }) => {
  ...
  if (isOfflineTab) {
    return <OfflineYearHeader year={year} />;
  }

  return <OnlineYearHeader year={year} />;
};

const OnlineYearHeader = ({ year }: { year: Year | null }) => {
  return (
    <View className="flex w-full flex-col items-center gap-1 py-2">
      ...
      <RelistenText className="text-l w-full text-center italic text-gray-400">
        <Plur word="show" count={year?.showCount} /> &middot;&nbsp;
...
const OfflineYearHeader = ({ year }: { year: Year | null }) => {
